...
export function SessionProvider({ children }) {
  ...
  const { selectedExercise, selectExercise, clearSelectedExercise } =
  ...
  const [statusMsg, setStatusMsg] = useState(
    'Camera offline. Click "Enable Camera" to begin.',
  );
  ...
  const loadAnalyticsData = useCallback(async () => {
    ...
    try {
      ...
      const mobility =
        ...
        Array.isArray(mobilityResult.value)
      ...
      const avgMobility =
        mobility.length > 0
          ? Math.round(
              mobility.reduce((sum, item) => sum + item.score, 0) /
                mobility.length,
            )
          : 82;
  ...
  const endSession = useCallback(async () => {
    ...
    const avgAccuracy =
      accuracyHistory.length > 0
        ? Math.round(
            accuracyHistory.reduce((sum, val) => sum + val, 0) /
    ...
    try {
      ...
      console.warn(
        "[SessionContext] Analytics re-fetch failed, applying local updates:",
        err.message,
      );
        ...
        setWeeklyExercises((prev) =>
          prev.map((item) =>
            item.category === selectedExercise.category
              ? {
                  ...item,
                  completed: Math.min(item.target, item.completed + 1),
                }
              : item,
          ),
        );
      ...
      setProgressData((prev) => {
        ...
        if (idx !== -1) {
          ...
          updated[idx] = {
            ...
            score:
              Math.round(
                ((day.score * day.sessions + avgAccuracy) / nextSessions) * 10,
              ) / 10,
