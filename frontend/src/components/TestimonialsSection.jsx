...
export default function TestimonialsSection() {
  ...
  return (
    <section ref={sectionRef} className="py-xl md:py-32 bg-white scroll-reveal" id="testimonials">
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-xl">
        ...
        {loading ? (
          <div className="text-center text-on-surface-variant">Loading stories...</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-gutter">
            {testimonials.map((t, index) => (
              <div key={t.id} className="bg-surface p-xl rounded-2xl border border-outline-variant relative scroll-reveal" style={{ transitionDelay: `${index * 0.15}s` }}>
                ...
                <div className="flex gap-md items-center mb-md">
                  ...
                  <div>
                    ...
                    <div className="text-label-sm text-secondary uppercase">{t.role}</div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0b1c30]/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-surface rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-lg border-b border-outline-variant flex justify-between items-center bg-surface-container-low shrink-0">
              <h3 className="text-headline-md font-headline-md text-on-surface">Share Your Story</h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="flex flex-col min-h-0">
              <div className="p-lg flex flex-col gap-md overflow-y-auto">
                ...
                <div className="space-y-xs">
                  <label className="text-label-md text-on-surface font-bold">Your Name</label>
                  <input 
                    type="text" 
                    name="name" 
                    value={formData.name} 
                ...
                <div className="space-y-xs">
                  <label className="text-label-md text-on-surface font-bold">Your Role / Condition</label>
                  <input 
                    type="text" 
                    name="role" 
                    value={formData.role} 
                ...
                <div className="space-y-xs">
                  <label className="text-label-md text-on-surface font-bold">Profile Picture (Optional)</label>
                ...
                <div className="space-y-xs">
                  <label className="text-label-md text-on-surface font-bold">Your Story</label>
                  <textarea 
                    name="content" 
